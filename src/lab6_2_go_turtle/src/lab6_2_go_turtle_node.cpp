...
int main( int argc, char** argv )
{
    ...
    ros::Publisher second_marker_pub = n.advertise<visualization_msgs::Marker>("second_Visualization_marker", 10);
    ...
    second_marker.mesh_resource="file:///home/pinton/Documents/ros_labs/student1839m_ws/src/lab6_2_go_turtle/src/Turtle.obj";
    ...
    marker.mesh_resource="file:///home/pinton/Documents/ros_labs/student1839m_ws/src/lab6_2_go_turtle/src/Turtle.obj";
    ...
    flag_marker.mesh_resource="file:///home/pinton/Documents/ros_labs/student1839m_ws/src/lab6_2_go_turtle/src/beacon.obj";
    ...
    flag_marker_quat = tf::createQuaternionFromRPY(M_PI_2,0,M_PI_2);
    ...
    //Создаём обычный маркер для того, чтобы визуализировать его в среде
    ...
    //Добавляем в текущий интерактивный маркер созданный элемент визуализации

    //Создаём орган управления, позволяющий перемещать маркер
    ...
    //Добавляем орган управления к маркеру
    main_marker.controls.push_back(move_control);
    //Указываем функцию обратного вызова при перемещении маркера
    ...
    while (ros::ok())
    {
        ...
        if(flag1)
        {
                    ...
                    || abs(point_flag_marker.position.y-marker.pose.position.y)>0.5)
            {
                ...
                marker.pose.position.y+=delta_y*0.002;
                marker.pose.position.z+=delta_z*0.002;
        ...
        if(flag2)
        {
                    ...
                    || abs(point_flag_marker.position.y-second_marker.pose.position.y)>0.5)
            {
                ...
                second_marker.pose.position.y+=delta_y*0.002;
                second_marker.pose.position.z+=delta_z*0.002;
                double alpha = atan(delta_z/delta_y);
                tf::Quaternion qu;
                if(delta_y <= 0)
                {
